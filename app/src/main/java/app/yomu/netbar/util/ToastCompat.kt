 ...
 *
 * 日志如下: <pre> android.view.WindowManager$BadTokenException: Unable to add window -- token
 * android.os.BinderProxy@5d9e24c is not valid; is your activity running?
 * ```
 ...
 *      }
 * ```
 * </pre> 原因: 在Android N，Toast显示和隐藏交由NMS[NotificationManagerService]来处理，NMS最终调用WMS
 * [WindowManagerService]操作屏幕上的View 在toast显示时会创建当前toast所对应的token，用于和WMS交互
 * 在toast隐藏后会移除token，此时token算超时过期. 如果显示Toast的过程中碰到主线程阻塞（手机卡顿等原因）
 * 阻塞过久就会碰到正常显示的toast的token过期，等到显示的时候token已经过期，所以会抛出这个异常.
 *
 * 在Android N以后，Toast隐藏后加了11s[NotificationManagerService.FINISH_TOKEN_TIMEOUT]的延迟，然后再移除token，同时在
 * [Toast.TN.handleShow]内添加了try-catch包裹.
 * @see [NotificationManagerService.cancelToastLocked]
 *
 * 解决方案: 方案1. try-catch包裹[Toast.TN.handleShow]方法 反射修改mTN的mHandler的实例为我们自定义的实例，处理类型为[Toast.TN.SHOW]
 * 的消息，其他消息类型继续通过原来的handler处理.处理消息时使用try-catch包裹 </br> 方案2. 自定义Toast的上下文，重写
 * [Context.getSystemService]方法，将WMS进行一层包装，在[android.view.WindowManager.addView]添加try-catch
...
object ToastCompat {
    ...
    /**
     * 修复android N Toast 方案2 在Android N中，对Toast用到的WindowManager进行包装
     *
    ...
    private fun isCallFromShowToast(): Boolean {
        ...
        return indexOf != -1 &&
            indexOf + offset <
                stackTraces.size && // 判断当前栈位置下是否还有两个方法，分别是getSystemService和handleShow
    ...
    /**
     * 修复android N Toast, WindowManager$BadTokenException 方案1 修复android N 以上 Toast,
     * IllegalStateException [android.view.WindowManagerGlobal.addView]
     * ```
