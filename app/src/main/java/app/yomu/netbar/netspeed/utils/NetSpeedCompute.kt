package app.yomu.netbar.netspeed.utils

import app.yomu.netbar.netspeed.NetSpeedPreferences
import app.yomu.netbar.netspeed.stats.NetStats
import app.yomu.netbar.util.HandlerTicker
...
class NetSpeedCompute(private var onTick: OnTickNetSpeed? = null) {
    ...
    var interval: Int by
        Delegates.observable(NetSpeedPreferences.DEFAULT_INTERVAL) { _, old, new ->
            if (old != new) {
                refresh()
    ...
    private val handlerTicker: HandlerTicker =
        HandlerTicker(interval.toLong()) {
            synchronized(this) {
                refresh()
